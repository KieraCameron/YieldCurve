...
class Day:
    def __init__(self, date):
        self.date = date
        self.yield_values = list()
        self.maturity_dates = list()
        self.xticks_lengths = list() # some days have different maturity dates available, e.g., 30 Yr and 4 Mo, so we need each day to have it's own xticks list
    def __repr__(self):
    ...
    def set_xticks_lengths(self):
        for maturity_date in self.maturity_dates:
            self.xticks_lengths.append(X_AXIS_TICKS[maturity_date])

    def get_spread(self, short_maturity_date, long_maturity_date):
        short_yield = self.get_yield(short_maturity_date)
        long_yield = self.get_yield(long_maturity_date)
        if short_yield == None or long_yield == None:
            return None
        else:
            return long_yield - short_yield

    def get_yield(self, maturity_date):
        if maturity_date not in self.maturity_dates:
            return None
        index = self.maturity_dates.index(maturity_date)
        return self.yield_values[index]
...
def get_day(start_year=0, end_year=9999, skip=0):
    ...
    for s in range(SKIP+1): # if skip is 0, then add 1 so the loop runs once. 
        day = next(days)
...
def plot_yield_curve(i):
        mat.cla()
        day = get_day(START_YEAR, END_YEAR, SKIP)
        #mat.xlim(-5, 470)
        mat.ylim(*y_limits_per_year[day.date.year])

        # day.set_xticks_lengths()
        ...
        elif day.get_spread(MDate.THREE_MONTH, MDate.TEN_YEAR) < 0:
